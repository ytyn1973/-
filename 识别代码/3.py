...
# 实现图形识别功能的主函数
def get_contours(dst1, img):
    #try except结构使程序在识别不到图像的情况下不出问题
    try:
        contours, _ = cv2.findContours(dst1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)#提取轮廓
        contours = sorted(contours, key=cnt_area, reverse=True)#将提取的轮廓按照大小排序
        dst = cv2.drawContours(img, contours, 1, (11, 210, 255), 5)#画出最大轮廓
        cnt = contours[0]#将最大轮廓赋值
        peri = cv2.arcLength(cnt, True)#计算长度
        approx = cv2.approxPolyDP(cnt, 0.086 * peri, True)#找顶点，0.086是可调阈值

        if len(approx) == 4:#根据顶点个数给出图形
            print("Rectangle")
        else:
            print("Circle")
...
def work_thread_rgb82bgr(cam=0, pData=0, nDataSize=0):
    ...
    while True:
        ...
        if ret == 0:
            ...
            img3 = cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY)#灰度处理
            img = cv2.equalizeHist(img3)#直方图均衡化，可降低光照对识别影响
            t, dst1 = cv2.threshold(img, 100, 255, cv2.THRESH_BINARY)#二值化
            #cv2.imshow('temp1',img)
            get_contours(dst1, img)
            cv2.waitKey(1)
            #else:
